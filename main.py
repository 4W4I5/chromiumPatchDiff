...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("version", nargs="?", help="Chrome version (e.g., 146.0.7680.178). If omitted, an interactive menu is shown.")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--base-version",
        help="Optional base Chrome version for compare-scoped commit enrichment (e.g., 146.0.7680.165)",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--xlsx-output",
        help="XLSX report output path (default: reports/cve_enrichment_<version>.xlsx)",
    )
    parser.add_argument(
        "--print-json",
        action="store_true",
...
def _fetch_recent_chrome_versions(limit: int | None = None, verbose: bool = False) -> list[str]:
    ...
    # Fallback: scan all refs/tags links if the specific Tags column is absent or unexpectedly empty.
    if not versions:
        if verbose:
            print(
                f"{Fore.MAGENTA}[VERBOSE]{Style.RESET_ALL} "
...
def _select_version_interactively(config: PipelineConfig, verbose: bool = False) -> str | None:
    if verbose:
        print(
            f"{Fore.MAGENTA}[VERBOSE]{Style.RESET_ALL} "
    ...
    if versions:
        ...
        while True:
            print(f"{Fore.MAGENTA}No version supplied. Select major.minor branch first:{Style.RESET_ALL}")
    ...
    manual = input(f"{Fore.MAGENTA}Enter Chrome version manually (or q to quit): {Style.RESET_ALL}").strip()
